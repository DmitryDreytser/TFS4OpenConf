...
{
	if( 0 == _stricmp(FileName, "CVS")       ) return true; // CVS
	if( 0 == _stricmp(FileName, ".svn")		) return true; // svn
	if( 0 == _stricmp(FileName, ".git")		) return true; // git
	if( 0 == _stricmp(FileName, ".gitignore")) return true; // git
	if( 0 == _stricmp(FileName, "hg")		) return true; // mercurial
	if( 0 == _stricmp(FileName, ".hgignore") ) return true; // mercurial
	if( 0 == _stricmp(FileName, "ChangeLog") ) return true;
	if( 0 == _stricmp(FileName, "gcomp.ini") ) return true;
...
void Msg(int MinimalVerbosityLevel, char *msg, ...)
{
	if( MinimalVerbosityLevel <= TaskParameters.Verbose )
	{
		...
		case LogType_stdout:
			...
			printf("\r\n");
			fflush(stdout);
			//std::wcout << (CString)ConvertedMsg << std::endl;
			break;
