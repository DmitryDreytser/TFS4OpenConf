...
void CStreamInfo::Reset()
{
	...
	IsText = true; // текстов в конфигурации больше.
...
void CStreamInfo::UpdateInfo(LPCSTR StreamName)
{
	...
	else if ( 0 == strnicmp("Page.", StreamName, 5)           ) IsText = false;
	else if ( 0 == stricmp("GUDData", StreamName)            ) IsText = false;
	else if ( 0 == stricmp("TagStream", StreamName)          ) IsText = false;
}
...
unsigned long CStreamInfo::ReadSize(IStream* pStream)
{
	...
	//—позиционируемс€ на начало потока
	Offset.LowPart = 0;
	pStream->Seek(Offset, STREAM_SEEK_SET, NULL);
...
void CStreamInfo::DeterminePictureType(IStream* pStream) //этот метод работает при декомпил€ции
{
	if ( !HaveSize )
	{
...
void CStreamInfo::DeterminePictureType(GCString& FileName) // этот метод работает при компил€ции
{
	char ext[_MAX_EXT];
