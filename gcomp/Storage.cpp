...
bool CStorage::CopyToFile(FILE* File, unsigned long bytes_to_copy)
{
	...
	if ( TaskParameters.UnixEOL && StreamInfo.IsText )
	{
		/* this wonderfull piece of code was copyrighted from tofrodos project
		   Thank you, guys!
		*/
		char c[1];
		int prevch = EOF;
...
bool CStorage::CopyToFile(GCString& FileName, bool OnlyRestOfSream)
{
	...
	if( !CopyToFile(File, bytes_to_copy) )
	{
		if(13 == errno)
		{
			Msg(1, "%s: Файл '%s' только для чтения.", "SKP", FileName);
			//fclose(File);
			return true;
		}
		Msg(0, "Error writing to file '%s' - %s (%d)", FileName, strerror(errno), errno);
		throw NULL;
	}
...
unsigned long CStorage::AppendFile(IStream* pStream, FILE* File)
{
	...
	if ( TaskParameters.UnixEOL && StreamInfo.IsText )
	{
		// TODO This way of copying (one by one char) is slow (20 vs 12 sec on compile)
		char charbuff[1];
		int c;
		int prevch;
		prevch = EOF;
